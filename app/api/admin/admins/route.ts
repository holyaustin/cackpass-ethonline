...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!ctx.isSuperAdmin) return NextResponse.json({ error: 'Super admin only' }, { status: 403 })
    await connectDB()
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (normalized === SUPER_ADMIN_EMAIL.toLowerCase()) {
      return NextResponse.json({ error: 'Cannot re-grant super admin' }, { status: 400 })
    }

    const cleanPerms: Permission[] = (permissions || []).filter((p: string) =>
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (email === SUPER_ADMIN_EMAIL.toLowerCase()) {
      return NextResponse.json({ error: 'Cannot revoke super admin' }, { status: 400 })
    }
