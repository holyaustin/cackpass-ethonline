// app/api/admin/contract/update-owner/route.ts
import { NextRequest, NextResponse } from 'next/server'
import { ethers } from 'ethers'
import { requireAdmin, logAdminAction, AdminAuthError } from '@/lib/admin/auth'
import { PERMISSIONS } from '@/lib/admin/permissions'

const ABI = ['function updatePlatformOwner(address newOwner) external']

export async function POST(request: NextRequest) {
  try {
    const ctx = await requireAdmin(request, PERMISSIONS.UPDATE_OWNER)
    ...
    const tx = await contract.updatePlatformOwner(newOwner, { gasLimit: 100000 })
    await tx.wait()

    await logAdminAction(ctx, 'contract.updateOwner', newOwner, { txHash: tx.hash })
