...
async function fetchUSDCBalance(walletAddress: string): Promise<{
  ...
  try {
    ...
    // Fallback to Blockscout API if RPC fails
    try {
      console.log('🔄 Falling back to Blockscout API...')
      ...
      console.error('Blockscout API also failed:', apiError)
    }
...
export default function WalletPage() {
  ...
  // Initial load and when wallet address changes
  useEffect(() => {
  ...
  useEffect(() => {
    if (!walletAddress || !authenticated) {
      ...
      if (transactionCheckIntervalRef.current) {
        ...
        transactionCheckIntervalRef.current = null
      }
      return
    }
    ...
    balancePollIntervalRef.current = setInterval(() => {
      const now = Date.now()
      // Only poll if last check was more than 30 seconds ago
    ...
    // Set up transaction check interval (less frequent)
    transactionCheckIntervalRef.current = setInterval(() => {
      if (hasPendingTransaction) {
        console.log('🔄 Checking for pending transactions...')
        fetchTransactions(pagination.page)
      }
    }, BALANCE_POLL_INTERVAL * 2) // Check transactions every 60 seconds
  ...
  }, [walletAddress, authenticated, lastBalanceCheck, hasPendingTransaction, pagination.page, fetchBalance, fetchTransactions])
  ...
  const handleSendTransaction = async () => {
    ...
    try {
      ...
      toast.success('USDC Transfer sent!', {
        ...
        action: {
          label: 'View',
          onClick: () => window.open(`https://blockscout.lisk.com/tx/${tx.hash}`, '_blank')
        }
      })
      ...
      setIsSending(false)
      // Keep pending transaction flag for a bit longer for confirmation checks
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">
                Manage your USDC balance on Lisk Mainnet
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <p className="text-blue-600 dark:text-blue-400 mt-1">
                  ...
                  Make sure you're connected to Lisk Mainnet (Chain ID: 1135).
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4 ">
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-xl">
                ...
                <div className="text-sm text-text-light space-y-1 text-gray-700 dark:text-gray-300">
                  ...
                  <div className="flex justify-between">
                    <span>Chain ID</span>
                    <span className="font-mono">1135</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Token</span>
