...
// Production configuration for Arc Mainnet
...
export function validateProductionEnvironment(): { isValid: boolean; errors: string[] } {
  ...
  Object.entries(ARC_MAINNET_CONFIG.REQUIRED_ENV_VARS).forEach(([key, description]) => {
    if (!process.env[key]) {
      errors.push(`Missing ${key}: ${description}`)
    }
  })
  
  if (process.env.NEXT_PUBLIC_ARC_CHAIN_ID !== '5042') {
    errors.push(`Invalid chain ID: Expected 5042 (Arc Mainnet), got ${process.env.NEXT_PUBLIC_ARC_CHAIN_ID}`)
  }
